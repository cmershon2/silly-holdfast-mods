...
{
    public class ISyncAnimator_NetworkController : IHoldfastSharedMethods
    {
        private ISyncAnimator[] animatorSync;

        private string brandingColor = "#ff5e00";

        public void OnSyncValueState(int value) {
        }

        public void OnUpdateSyncedTime(double time) {
            foreach (ISyncAnimator actor in animatorSync)
            {
                actor.syncAnimationByValue((float)time, 0);
            }
        }
        ...
        public void OnIsClient(bool client, ulong steamId) {
            var actors = Resources.FindObjectsOfTypeAll<ISyncAnimator>();

            if(actors.Length > 0) {
                Debug.Log($"<color={brandingColor}>ISyncAnimator Network Controller</color> : Found {actors.Length} animator actors.");
                animatorSync = actors;
            } else {
                Debug.Log($"<color={brandingColor}>ISyncAnimator Network Controller</color> : No animator actors found.");
            }
        }
